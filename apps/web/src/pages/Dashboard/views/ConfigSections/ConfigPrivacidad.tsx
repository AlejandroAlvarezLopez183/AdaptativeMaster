...
interface ConfigPrivacidadProps {
  user: UserResponse | null;
  onBack: () => void;
  onUpdateUser?: (user: UserResponse) => void;
}

export function ConfigPrivacidad({ user, onBack, onUpdateUser }: ConfigPrivacidadProps) {
    ...
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            <h2 style={{ margin: 0, color: '#F5F3EE', fontSize: '20px', fontFamily: "'Fraunces', Georgia, serif" }}>Privacidad</h2>
            <p style={{ margin: '4px 0 0 0', color: '#8FA8AA', fontSize: '14px' }}>Controla quién puede ver tu actividad.</p>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          <Toggle 
            ...
            desc="Permite que otros estudiantes te encuentren y vean tu perfil."
            checked={perfilPublico} onChange={setPerfilPublico} 
          />
          <Toggle 
            label="Mostrar Progreso" 
            desc="Muestra tus logros y avance en las tablas de clasificación."
            checked={mostrarProgreso} onChange={setMostrarProgreso} 
          />
        </div>

        {error && <div style={{ color: '#F2637B', fontSize: '14px' }}>{error}</div>}
        {success && <div style={{ color: '#4CAF50', fontSize: '14px' }}>¡Privacidad actualizada!</div>}
