...
export function AprendizajeView({ setActive }: AprendizajeViewProps) {
  ...
  const cursos = [
    { id: 1, titulo: "Python Backend", icon: "🐍", progreso: 72, lecciones: "12/17" },
    { id: 2, titulo: "PostgreSQL", icon: "🗄", progreso: 34, lecciones: "5/15" },
  ];
        ...
        {tab === "en_curso" && cursos.map(curso => (
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 8 }}>
              <span style={{ fontSize: 13, color: '#8FA8AA' }}>{curso.lecciones} lecciones</span>
              <span style={{ fontSize: 18, fontWeight: 700, color: '#E8B94A' }}>{curso.progreso}%</span>
            </div>
            
            <div style={{ width: '100%', height: 8, background: 'rgba(0,0,0,0.3)', borderRadius: 4, overflow: 'hidden', marginBottom: 24 }}>
              <div style={{ width: `${curso.progreso}%`, height: '100%', background: 'linear-gradient(90deg, #C49A33, #E8B94A)', borderRadius: 4 }} />
            </div>
